...
#ifdef _DEBUG
#if _MSC_VER > 1200
	sprintf(PluginInfo->Name, "%s Debug %d.%d.%d",project_name, FILE_VERSION0,FILE_VERSION1,FILE_VERSION2);
#else
	sprintf(PluginInfo->Name, "%s Debug %d.%d.%d - Win98",project_name, FILE_VERSION0,FILE_VERSION1,FILE_VERSION2);
#endif
#else
#if _MSC_VER > 1200
	sprintf(PluginInfo->Name, "%s %d.%d.%d",project_name, FILE_VERSION0,FILE_VERSION1,FILE_VERSION2);
	//sprintf(PluginInfo->Name, "%s %d.%d.%d (%s)",project_name, FILE_VERSION0,FILE_VERSION1,FILE_VERSION2,BUILD_DATE);
#else
	sprintf(PluginInfo->Name, "%s %d.%d.%d - Win98",project_name, FILE_VERSION0,FILE_VERSION1,FILE_VERSION2);
#endif
#endif
...
__declspec(dllexport) void CALL DllAbout ( HWND hParent )
{
	char temp[300];
	sprintf(temp,"%s %d.%d.%d (build %d) \nDirectX 8.1+\nOpenGL 1.1-1.4/ATI/Nvidia TNT/Geforce Extension",project_name, FILE_VERSION0,FILE_VERSION1,FILE_VERSION2,BUILD_NUMBER) ;
	MsgInfo(temp);
}
...
__declspec(dllexport) void CALL DllTest ( HWND hParent )
{
	MsgInfo("TODO: Test");
}

__declspec(dllexport) void CALL DllConfig ( HWND hParent )
{
...
__declspec(dllexport) void CALL ChangeWindow ()
{
...
__declspec(dllexport) void CALL DrawScreen (void)
{
}

//---------------------------------------------------------------------------------------

__declspec(dllexport) void CALL MoveScreen (int xpos, int ypos)
{ 
}
...
//---------------------------------------------------------------------------------------
__declspec(dllexport) void CALL RomClosed (void)
{
...
__declspec(dllexport) void CALL RomOpen (void)
{
...
__declspec(dllexport) void CALL UpdateScreen (void)
{
#ifdef USING_THREAD
	if (videoThread)
	{
...
__declspec(dllexport) void CALL ViStatusChanged (void)
{
	g_CritialSection.Lock();
	SetVIScales();
...
//---------------------------------------------------------------------------------------
__declspec(dllexport) void CALL ViWidthChanged (void)
{
	g_CritialSection.Lock();
	SetVIScales();
	CRender::g_pRender->UpdateClipRectangle();
...
__declspec(dllexport) BOOL CALL InitiateGFX(GFX_INFO Gfx_Info)
{
...
__declspec(dllexport) void CALL CloseDLL (void)
{ 
...
__declspec(dllexport) uint32 CALL ProcessDListCountCycles(void)
{
#ifdef USING_THREAD
	if (videoThread)
	{
...
__declspec(dllexport) void CALL ProcessRDPList(void)
{
#ifdef USING_THREAD
	if (videoThread)
	{
...
__declspec(dllexport) void CALL ProcessDList(void)
{
#ifdef USING_THREAD
	if (videoThread)
	{
...
/******************************************************************
  ...
  output:   none
*******************************************************************/ 
__declspec(dllexport)  void CALL FBWList(FrameBufferModifyEntry *plist, uint32 size)
{
}
...
__declspec(dllexport)  void CALL FBRead(uint32 addr)
{
...
__declspec(dllexport) void CALL FBWrite(uint32 addr, uint32 size)
{
...
//__declspec(dllexport) void CALL FBWrite2(uint32 addr, uint32 size)
//{
//	FrameBufferWriteByCPU(addr, size);
//}
