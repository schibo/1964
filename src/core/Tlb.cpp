...
void r4300i_COP0_tlbp(uint32 Instruction)
{
	...
	for(index = 0; index <= NTLBENTRIES; index++)
	{
		 ...
		 * mask with TLB.MyHiMask ?		 * I think should just mask with TLBHI_VPN2MASK, not with TLB.MyHiMask according
		 * to r4300i manual ?		 * But doing this, some game will give errors of TLBP lookup fail. ?		 * if( ( (gMS_TLB[index].EntryHi & TLBHI_VPN2MASK) == (gHWS_COP0Reg[ENTRYHI] &
		 * TLBHI_VPN2MASK) ) &&
		 */
		if
		(
...
void tlb_write_entry(int index)
{
	 ...
	 * not both of them ?	 * yes, still need in Direct TLB lookup
	 */
	 ...
	 * the ENTRYHI register ?	 * we will not store the g bit in the EntryHi, but keep them in EntryLo0 and
	 * EntryLo1 ?	 * theTLB->EntryHi = (gHWS_COP0Reg[ENTRYHI] & (~(uint32)gHWS_COP0Reg[PAGEMASK])) |
...
{
	TLB_TRACE(TRACE1("TLB write to 0x%08x", address));
...
uint32 RedoTLBLookupAfterException(uint32 address, int operation)
{
	...
	for(c = 0; c < MAXTLB; c++)
	{
		...
		/*
		 * skip unused entries ?		 * if (theTLB->valid == 0) continue; ?		 * if ( ((theTLB->EntryLo0 | theTLB->EntryLo1)) == 0) continue; ?		 * compare upper bits ?		 * if ((address & theTLB->MyHiMask & 0x1FFFFFFF) == (theTLB->EntryHi &
		...
		if((address & theTLB->MyHiMask) == (theTLB->EntryHi & theTLB->MyHiMask))
		{
			...
			)	/* ASID match */
			{
				...
				if(EntryLo & 0x02)
				{
					...
					TLB_TRACE(TRACE2("TLB Map VA =%08X to Addr= %08X", address, realAddress));
					return realAddress;
				}
				else
				{
	...
	TLB_TRACE(TRACE1("Redo TLB check still fail, mapped to dummy memory, VA =%08x", address));
	if(operation == TLB_INST)
	{
		TLB_TRACE(TRACE1("Instruction TLBL Missing Exception, Bad VA = 0x%08X", address));
		DisplayError
		(
...
uint32 TranslateTLBAddress(uint32 address, int operation)
{
	...
	/* for (c = 0; c < MAXTLB; c++) */
	{
		 ...
		 * if ( ((theTLB->EntryLo0 | theTLB->EntryLo1) & TLBLO_V ) == 0) continue; // Skip
		 // this entry if both EntroLo0 and EntryLo1 are invalid ?		 // compare upper bits ?
		 /* This is a unreasonable hack, I should not use this 0x1FFFFFFF mask, but ?		 * I cannot find the bugs in dyna, without this mask, many games will not work
		 * well ?		 * games like Snowboard Kids 2 ?*/
		...
		if((address & theTLB->MyHiMask) == (theTLB->EntryHi & theTLB->MyHiMask))	/* match, this line is correct */
		{
			...
			)						/* ASID match */
			{
				...
				if(address & theTLB->LoCompare)
				{
					Entry  = theTLB->EntryLo1;
					Entry2 = theTLB->EntryLo0;
				}
				else
				{
					Entry  = theTLB->EntryLo0;
					Entry2 = theTLB->EntryLo1;
				}

				if(Entry & 0x02)	/* Is the entry valid ?? */
				{
					/* calculate the real address from EntryLo */
					realAddress |= ((Entry << 6) & ((theTLB->MyHiMask) >> 1));
					realAddress |= (address & ((theTLB->PageMask | 0x00001FFF) >> 1));
...
uint32 Trigger_TLB_Refill_Exception(uint32 address, int operation)
{
	...
	if(debugoptions.debug_tlb)
	{
		...
		{
			TRACE2("0x%08X: TLBL Refill Exception, VA= %08X", gHWS_pc, address)
		}
		else if(operation == TLB_STORE)
		{
			TRACE2("0x%08X: TLBS Refill Exception, VA= %08X", gHWS_pc, address)
		}
		else
		{
			TRACE2("0x%08X: ITLB Refill Exception, VA= %08X", gHWS_pc, address)
		}
	}
#endif
	...
	/*
	 * Test the TLB refill service vector ?	 * Hack for Turok: Rage War and for NFL QBC: 2000, donno why I need this ?	 * For the TLB service vector here, it is tricky. ?	 * I should use 0x80000000 to serve TLB_Refill for 32bit TLB, and use 0x80000080
	 * to serve ?	 * 64bit TLB. I believe I should use 0x80000000 for most of games, ?	 * Somehow, Golden Eye only work if I use 0x80000000, while Turok 2 only work if I
	 * use ?	 * 0x80000080, do I need to check the status register to decide the vector? ?	 * if( TLB_Refill_Exception_Vector != 0x80000000 )
	 */
	{
...
step2:
	...
	else
	{
		...
		TLB_Error_Vector = TLB_Refill_Exception_Vector;
		TLB_TRACE(TRACE1("Call HandleException for Refill, vaddr=%08X", TLB_Error_Vector));
		...
		if(emustatus.cpucore == DYNACOMPILER)
		{
			...
			{
				uint32 newaddr = TranslateTLBAddress(address, operation);
				TLB_TRACE(TRACE2("TranslateTLBAddress, VA=%08X to %08X", address, newaddr));
			...
			else
			{
				...
				TLB_TRACE(TRACE1("RedoTLBLookupAfterException, vaddr=%08X", address));
				return(RedoTLBLookupAfterException(address, operation));
			}
		}
		else
		{
			TLB_TRACE(TRACE1("TLB Refill for load/store, vaddr=%08X", address));
...
uint32 Trigger_TLB_Invalid_Exception(uint32 address, int operation)
{
	...
	if(debugoptions.debug_tlb)
	{
		if(operation == TLB_LOAD)
		{
			TRACE2("0x%08X: TLBL Invalid Exception, VA = 0x%08X", gHWS_pc, address)
		}
		else if(operation == TLB_STORE)
		{
			TRACE2("0x%08X: TLBS Invalid Exception, VA = 0x%08X", gHWS_pc, address)
		}
		else
		{
			TRACE2("0x%08X: ITLB Invalid Exception, VA=%08X", gHWS_pc, address)
		}
	}
#endif
	gHWS_COP0Reg[BADVADDR] = address;
	...
	else
	{
		...
		if(emustatus.cpucore == DYNACOMPILER)
		{
			if(newtlb)	/* if a new TLB entry is written in the TLB exception service routine */
			{
				/* Could make infinite loop, be careful here */
				uint32 newaddr = TranslateTLBAddress(address, operation);
				TLB_TRACE(TRACE2("Translate VA=%08X to Addr=%08X", address, newaddr));
				return newaddr;
			}
			else
			{
...
uint32 TLBMapErrorAddress(uint32 address)
{
	...
	__try
	{
		...
		dummyAddr = ((address & 0x1fffffff) | 0x80000000);
	}
	...
	__except(NULL, EXCEPTION_EXECUTE_HANDLER)
	{
		...
		{
			dummyword = LOAD_UWORD_PARAM(address);
			dummyAddr = (address & 0x1fffffff);
		}

		__except(NULL, EXCEPTION_EXECUTE_HANDLER)
		{
			dummyAddr = ((address & 0x0000FFFF + 0x1FFF0000) | 0x80800000);	/* mapped to dummy segment */
		}
	}
	TLB_TRACE(TRACE2("Map VA=%08X to Dummy Addr = %08X", address, dummyAddr));
	//TLB_TRACE(TRACE2("Map VA=%08X to Dummy Addr = %08X", address, 0x80800000));
	return dummyAddr;
	//return 0x80800000;
}
...
//#define CHECK_CACHE
#ifdef CHECK_CACHE 
	int QuerAddr; int size;
	if( currentromoptions.Code_Check !=	 CODE_CHECK_PROTECT_MEMORY ) 
	{ 
		uint32 op = RT_FT; 
		int code = op/4; 
		int cache = op&1; 
		int size = 0x10; // size for data cache 
		
       
        STORE_TLB_FUN; 
		MessageBox(0, "Cache", "", 0);
        
        QuerAddr &=	 0xFFFFFFF0; 
		if( cache == 0 ) //Instruction cache 
		{ 
			size = 0x20; QuerAddr &= 0xFFFFFFE0; 
		} 
		//QuerAddr &= 0xFFFFC000; // align it to 16kb blocks, ?? 
		if(QuerAddr >= 0x80000000 && QuerAddr < 0x80000000+current_rdram_size )
		if(QuerAddr >= 0x80004000 && QuerAddr < 0x80000000+current_rdram_size ) 
		{ 
			if( code == 6 || ((code == 0 || code == 5) && cache == 1) ) 
			{ 
				uint32 pc;
				DEBUG_CACHE_TRACE(TRACE3("Cache, code=%d, cache=%d, addr=%08x", op/4, (op&3), QuerAddr)); 
			
				for(pc= QuerAddr; pc<QuerAddr+size; pc+=4) 
				{ 
					if((uint32*)(RDRAM_Copy+(pc&0x007FFFFF)) != *(uint32*)(gMS_RDRAM+(pc&0x007FFFFF))) 
					{ 
						if( gHardwareState.sDYN_PC_LOOKUP[pc>>16] != gMemoryState.dummyAllZero )
							*(uint32*)((uint8*)gHardwareState.sDYN_PC_LOOKUP[pc>>16] + (uint16)pc) = 0; 
						else break; 
					} 
				} 
		} 
...
void Refresh_Direct_TLB_Lookup_Table(int index)
{
	 ...
	 * TLB, but not ?	 * change TLB all the time. At least games should not change the TLB in used all
	 * the time while ?	 * leave the TLB entries not in used untouched forever. ?	 * But I know some games are just constantly used only a few TLB ?	 * TLB_TRACE(TRACE0("Refresh Direct TLB Lookup Table"))
	 */
	if(index > NTLBENTRIES)
	{
	...
	else
	{
		...
		/* case1: If the new TLB is invalid and the old TLB is also invld, do nothing */
		...
		/*
		 * case3: If the old TLB is invld, and the new TLB is valid, do something,
		 * without checking ?		 * other TLB entries
		 */
		...
		/*
		 * case4: If the old TLB is valid and new TLB is invld, do something, need to be
		 * careful
		 */
		if(new_valid == 0)
		{
		 ...
		 * refresh the whole ?		 * TLB lookup table, this will be a little slower ?		 * If I can do better, I prefer to exam the conflict TLB entries, and modify the
		 * TLB Lookup ?		 * table only if needed ?		 * here is the easiest solution ?		 * 1. Write new TLB into TLB table (not Direct TLB Lookup Table) ?		 * tlb_write_entry(index); ?		 * 2. Refresh the whole Direct TLB Lookup Table ?		 * Build_Whole_Direct_TLB_Lookup_Table();
		 */
	}
}
...
void Build_Direct_TLB_Lookup_Table(int index, BOOL tobuild)
{
	...
	if(theTLB->EntryLo0 & 0x2)					/* is the TLB EntryLo is valid? */
	{
		...
		/*
		 * realAddress |= ((EntryLo << 6) & ((theTLB->MyHiMask) >> 1)); ?		 * realAddress |= (address & ((theTLB->PageMask | 0x00001FFF) >> 1));
		 */
		...
		for(i = lowest / 0x1000; i < loop_optimizer; i++)
		{
			...
			else
			{
				...
				if(gHardwareState.sDYN_PC_LOOKUP[i >> 4] != gMemoryState.dummyAllZero)
				{
					memset(((uint8 *) gHardwareState.sDYN_PC_LOOKUP[i >> 4] + (uint16) (i * 0x1000)), 0, 0x1000);
				}
			}
		}
	}
...
void Build_Whole_Direct_TLB_Lookup_Table(void)
{
	...
	/*
	 * Clean the whole table first ?	 * memset(&Direct_TLB_Lookup_Table[0], 0xFF, sizeof(Direct_TLB_Lookup_Table));
	 */
	for(i = 0; i < 0x100000; i++)
	{
		Direct_TLB_Lookup_Table[i] = DUMMYDIRECTTLBVALUE; 
